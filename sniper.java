...
public class sniper extends d20weaponsystem
{
	//-------------------------------------------------------------------
	...
	static Random randomBaseSniper = new Random();
	String[] effectArray = {"","",""};
	String effect="", effectIntensity="", effectChance="";

	//-------------------------------------------------------------------
	//Begin sniperStats Method
	//-------------------------------------------------------------------
	public String[] sniperStats(int lvlI)
	{
		lvlInt = lvlI; //gets level
		damageModInt = 0; //sets the damage modifier variable to 0

		int randomBaseIntSn = randomBaseSniper.nextInt(100)+1;
		if(randomBaseIntSn<=25){
			manuName = "M:I ";
			baseName = "Diamondback ";
			weapName = "M:I Sniper Rifle";
			dmgDiceModel = "V";
		...
		else if(randomBaseIntSn>25 && randomBaseIntSn<=43){
			manuName = "Bohr ";
			baseName = "Widow-Maker ";
			weapName = "Bohr Sniper Rifle";
			dmgDiceModel = "G";
		...
		else if(randomBaseIntSn>43 && randomBaseIntSn<=56){
			manuName = "Arista ";
			baseName = "Sentinel ";
			weapName = "Arista Sniper Rifle";
			dmgDiceModel = "G";
		...
		else if(randomBaseIntSn>56 && randomBaseIntSn<=64){
			manuName = "Arista ";
			baseName = "Ace ";
			weapName = "Arista Special Sniper Rifle";
			dmgDiceModel = "G";
		...
		else if(randomBaseIntSn>64 && randomBaseIntSn<=89){
			manuName = "MechTec ";
			baseName = "Reaver ";
			weapName = "MechTec Sniper Rifle";
			dmgDiceModel = "L";
		...
		else if(randomBaseIntSn>89 && randomBaseIntSn<=100){
			manuName = "Rubicore ";
			baseName = "Cutlass ";
			weapName = "Rubicore Sniper Rifle";
			dmgDiceModel = "G";
		...
		if(randomAccModInt<= (15+((lvlInt-1)*4))){
		...
		if(randomDamModInt<= (10+((lvlInt-1)*3))){
			...
			if(randomBase.nextInt(100)+1<= (50+((lvlInt-1)*3)) ){
				...
				if(randomDamModInt2<=lvlModInt1){
					...
					if(randomBase.nextInt(100)+1<= 50){
						FRModInt = FRModInt-1;
						FRDisp = FRDisp-1;
						rarityInt = rarityInt-20;
					}
		...
		//---------------------------------------------------------------------------------------------------------------
		//Fire Rate Modification *FIRE RATE NOT MODIFIED HERE, REFER TO DAMAGE MODIFICATION*
		//---------------------------------------------------------------------------------------------------------------
		...
		if(randomScModInt<= (15+((lvlInt-1)*4))){
			if(randomScModInt2<=50){
		...
		if(randomCritModInt<=(5+((lvlInt-1)*3))){
			critTxt = true;
			rarityInt = rarityInt+30;
		}
		...
		displayText = displayText+"Sniper\n"+weapName+"\nDamage: "+damageBase+"\nAccuracy: "+accuracyBase+"\nMagazine: "
			+magazineBase+"\nFire Rate: "+firerateBase+"\nRange: "+rangeBase+" ft.\nMinimum Range: "+minrng+" ft.\nReload: "+reloadBase+"\nWeight: "+wgtBase
			+"\n"+descriptionTxt+"\n";

		displayArray[0] = "Sniper Rifle";
